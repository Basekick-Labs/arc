...
**This corrects new clusters only.** A node already recorded as a voter stays one, and re-joining does not change that: adding a server that is already a voter as a non-voter updates its address and leaves its suffrage alone. So an existing cluster keeps exactly the voters it has, including its readers, until a membership reconcile converges it. That is tracked separately, and deliberately so: the promotion half of such a reconcile is the one path that can leave a cluster permanently leaderless, and Arc has no cluster-recovery path to undo it.

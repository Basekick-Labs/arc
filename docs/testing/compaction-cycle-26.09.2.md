# Compaction cycle acceptance — 26.09.2
...
Recorded local run: [UTC-stamped results](compaction-cycle-26.09.2-results.json).
The record includes the tested binary digest, unchanged resource settings, cycle
outcomes, file counts, planned restarts, and observed memory-event deltas.
...
| Narrow scope modifies unrelated data | Recovery selects tier/database prefixes and verifies measurement metadata before mutation |
| Similar names or namespace paths bypass scope | Tests cover database and measurement prefixes, tiers, duplicate selections, empty scope, and spoke namespaces |
...
| Unreadable manifest exposes files to another compaction | Recovery retains it; cache construction fails closed instead of publishing partial state |
...
go test -tags=duckdb_arrow -race ./internal/compaction \
  -run 'Cycle|RecoveryScope|RecoveryUnreadable|RecoveryAggregates|RecoveryEmpty|ManifestCacheProtects' -count=20

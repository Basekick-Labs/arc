...
- **Ingest signature reverted to names-only.** The schema-change signature no longer encodes column types, so a column whose type varies across batches stays in one buffer (coerced or surfaced at write time) instead of silently fanning out into type-conflicting files. The allocation optimization from #411 is preserved; there is no added ingestion overhead.
- **`time` is forced to an integer-microsecond timestamp at ingest.** Every typed write coerces `time` to `int64` (→ Arrow `Timestamp`); a `time` column sent as a string is now rejected with a clear error at ingest rather than written as a `VARCHAR` column that would corrupt the partition's schema.
- **Compaction is now resilient to already-written mixed-type partitions.** The compaction query normalizes `time` to `TIMESTAMP` (`COALESCE(TRY_CAST(time AS TIMESTAMP), make_timestamp(TRY_CAST(time AS BIGINT)))`), so partitions that already contain a mix of `TIMESTAMP` and `VARCHAR` `time` files from an affected 26.05.1 deployment will compact successfully on the next cycle without manual intervention.

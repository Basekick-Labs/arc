...
**Performance.** Bench (`internal/cluster/replication/auth_bench_test.go`): per-entry tag compute is **391 ns / 552 B / 8 allocs** at a 256-byte payload (Apple M3 Max), trending up to ~3 µs at 8 KB payloads. For Arc's typical batch-of-records-per-entry shape (~100 records/entry → ~200k entries/sec at 19.9M records/sec), the per-entry overhead works out to ~0.08% of a single core. The full HMAC fires once every 1024 entries, so its cost amortises away.

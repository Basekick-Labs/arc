...
- **Line protocol timestamp validation**: The parser now enforces strict bounds on raw timestamp values before applying precision multipliers (`ms`, `s`). Values that would overflow `int64` after multiplication are rejected and replaced with the server's current time, preventing silently mis-timestamped records in edge cases.

**Performance**

- **Column signature caching**: Schema change detection computes a sorted, joined column name string on every write to determine whether the buffer schema has changed. This string is now computed once when the typed column batch is constructed and cached as a field. On the hot path (stable schema, the common case), the lookup is a field read with zero allocation.
...
- **`BatchToColumnar` tag lookup reduction**: Tag-key existence checks in the columnar conversion path were performed twice per field per record — once during schema discovery and once during data filling. A per-record `tagSet` is now built once and reused for both passes.

...
Four follow-ups from the same audit ride in this release: spoke **discovery** loads the ledger's known paths in one query instead of one point-lookup per stored file (100k files used to mean 100k queries per pass on the shared SQLite handle), the sync pass **pages by keyset cursor** instead of re-fetching every previously-offered row each page (O(pages²) row scans on a large backlog), a **concurrent bundle import now fails fast with `409 import in progress`** instead of silently queueing behind a run that can legally take hours, and in cluster mode received files stamp the manifest's `OriginNodeID` with the **receiving node** (matching the compaction bridge) rather than the spoke ID — which also makes them visible to local-storage reconciliation's per-node scoping; the spoke identity remains the path's first segment.
...
`GET /api/v1/spoke-sync/ledger` also takes `?state=` (any single state) — the way to enumerate `skipped` entries, whose three classes (vanished source, compacted output, operator-dismissed) are told apart by their note; the default view keeps hiding them.
...
**Compaction on a syncing spoke defers until the data has been delivered** (`edge_sync.spoke.defer_compaction_until_synced`, default `true`). Local compaction only consumes files the sync ledger reports delivered — synced over the network, or acked back on the air-gap path — and its outputs are recorded as already-delivered content that never syncs. Together those make hub-side row duplication and compaction-caused data loss structurally impossible: the hub receives every row exactly once, as raw files. What an operator sees: a spoke with a sync backlog logs a per-scan deferral line (broken out by ledger state — `exported` means a drive is awaiting its ack; `failed`/`conflicted` need attention) and its partitions compact on the first cycle after delivery. Setting the key `false` restores the old behavior — compact freely, accept hub duplicates — and invalidates the gate's bookkeeping epoch, so flipping back to `true` later treats anything compacted during the ungated period as legacy: it syncs once (a bounded duplicate), never silently disappears. Two notes: compacted files that predate the upgrade sync once (their rows may exist nowhere else — a possible one-time duplicate for old partitions, never a loss), and duplicates already accumulated on a hub are not retroactively cleaned.

**The hub compacts what it receives.** With `edge_sync.compact_received_namespaces` (default `true`), the hub's own compaction now processes spoke namespaces like any other data: registered spoke IDs expand into per-database compaction targets, and the small raw files a spoke delivers are folded into ordinary compacted Parquet on the hub's schedule. The sync protocol stays truthful throughout — receipts for compacted-away files are *marked*, never forgotten, so a spoke re-offering one (a pruned ledger being rediscovered, or a stale air-gap drive) is answered "already present" with zero bytes moved instead of re-uploading a duplicate next to the compacted output; a different-content re-delivery still conflicts. Set the key `false` to keep received data in its raw per-file layout (forensics, external tooling). Two scoping notes: keep spoke registrations for as long as their data sits in storage (an unregistered namespace is not expanded), and on a multi-node cluster enable this only where the spoke-facing endpoint runs on the compactor node — receipt bookkeeping is node-local in this release.

...
var arrowJSONQueryFunc func(h *QueryHandler, c *fiber.Ctx, ctx context.Context, cancel context.CancelFunc, convertedSQL string, profileMode bool, governanceMaxRows int, start time.Time, timestamp string, onComplete func(int)) (int, bool)
...
localProcessing:
	...
	if parallelInfo != nil && h.parallelExecutor != nil {
		...
		if arrowJSONQueryFunc != nil {
			...
			_, handled := arrowJSONQueryFunc(h, c, ctx, cancel, convertedSQL, profileMode, governanceMaxRows, start, timestamp, onComplete)
			if handled {
				// Arrow path handled the response — Complete is called with the real
				// row count inside the async stream writer callback via onComplete.
				return nil
			}
...
func (h *QueryHandler) queryMeasurement(c *fiber.Ctx) error {
	...
	if arrowJSONQueryFunc != nil {
		ctx := context.Background()
		_, handled := arrowJSONQueryFunc(h, c, ctx, nil, convertedSQL, false, 0, start, timestamp, nil)
		if handled {
